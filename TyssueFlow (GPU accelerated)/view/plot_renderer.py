...
def render_mesh_to_axis(mesh, ax, show_forces=False, title="Tissue Workspace"):
    bg_color = '#1e1e1e'; edge_color = '#4fc1ff'
    vert_color = '#ff6b6b'; text_color = '#d4d4d4'; arrow_color = '#facc15'
    
    ax.clear()
    ax.set_facecolor(bg_color)
    
    # --- DYNAMIC MAGMA COLORMAP FOR NOTCH (NICD) ---
    cmap = cm.magma
    norm = mcolors.Normalize(vmin=0, vmax=800)
    
    # 1. RENDER BOLD FACES WITH BIOCHEMICAL COLORING
    for f in range(mesh.num_faces):
        ...
        if len(poly_coords) > 2:
            # Map the face color to the NICD concentration (face_I)
            face_val = mesh.face_I[f]
            dynamic_color = cmap(norm(face_val))
            
            poly = patches.Polygon(poly_coords, closed=True, facecolor=dynamic_color, edgecolor=edge_color, lw=2.0, alpha=0.9)
            ax.add_patch(poly)
    ...
    if mesh.num_verts > 0:
        ax.scatter(mesh.vert_x[:mesh.num_verts], mesh.vert_y[:mesh.num_verts], color=vert_color, s=70, zorder=3, edgecolors='white', linewidths=1.0)
