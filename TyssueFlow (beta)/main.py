...
from core.boundaries import FixedBoundaryTopology, PeriodicBoundaryTopology
...
class MainDashboardApp:
    ...
    def _init_tk_variables(self):
        ...
        self.var_boundary_type = tk.StringVar(value="Fixed")  # Added boundary tracker variable
        ...
        self.var_p0 = tk.DoubleVar(value=0.0)      # Default requested
        self.var_p_ratio = tk.DoubleVar(value=3.6) # Default requested
    ...
    def _build_sidebar_widgets(self):
        ...
        # Boundary Configuration Toggle Layout
        boundary_frame = tk.Frame(m_box, bg=self.styles['bg_panel'])
        boundary_frame.pack(fill=tk.X, pady=5, padx=5)
        tk.Label(boundary_frame, text="Boundary:", bg=self.styles['bg_panel'], fg=self.styles['fg_text'], font=('Helvetica', 13)).pack(side=tk.LEFT)
        ttk.Radiobutton(boundary_frame, text="Periodic", variable=self.var_boundary_type, value="Periodic").pack(side=tk.RIGHT, padx=5)
        ttk.Radiobutton(boundary_frame, text="Fixed", variable=self.var_boundary_type, value="Fixed").pack(side=tk.RIGHT, padx=5)
    ...
    def _read_ui_parameters(self) -> bool:
        try:
            ...
            self.p_cfg.text_boundary = self.var_boundary_type.get()
    ...
    def trigger_initialization(self):
        try:
            if not self._read_ui_parameters(): 
                return
            ...
            # Strategy Pattern Resolution mapping to mesh_factory initialization
            if self.var_boundary_type.get() == "Periodic":
                boundary_strategy = PeriodicBoundaryTopology()
            else:
                boundary_strategy = FixedBoundaryTopology()

            self.tissue = build_lattice(
                self.m_cfg.grid_type, 
                self.m_cfg.nx, 
                self.m_cfg.ny, 
                self.m_cfg.noise_percent,
