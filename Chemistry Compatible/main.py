...
class MainDashboardApp:
    def __init__(self, root: tk.Tk) -> None:
        self.root = root
        self.root.title("TyssueFlow Pro - Mechanochemical Engine")
    ...
    def _init_tk_variables(self):
        ...
        self.var_dt_chem = tk.DoubleVar(value=self.bio_cfg.dt_chem)
    ...
    def _build_sidebar_widgets(self):
        ...
        self.sidebar.add_slider(b_box, "Chem Time Step (dt_chem):", self.var_dt_chem, 0.01, 0.50, 0.01)
        
        s_box = self.sidebar.add_label_frame(self.sidebar.inner_frame, " Global Viscous Solver ")
        self.sidebar.add_slider(s_box, "Mech Time Step (dt):", self.var_dt, 0.01, 0.20, 0.01)
    ...
    def _read_ui_parameters(self) -> bool:
        try:
            ...
            self.bio_cfg.dt_chem = self.var_dt_chem.get()
    ...
    def step_one_frame(self):
        ...
        swaps, energy = advance_simulation_frame(
            self.tissue, self.p_cfg, self.t1_cfg, self.bio_cfg, 
            self.m_cfg.grid_type, global_dt, mech_substeps, chem_substeps
        )
        
        self.current_time += global_dt
    ...
    def _sync_and_redraw(self, status_msg):
        ...
        self.ax_energy.set_xlabel("Actual Time (Minutes)", color=self.styles['fg_text'], fontsize=12, fontweight='bold')
