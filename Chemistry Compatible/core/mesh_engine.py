...
class TissueMesh:
    def __init__(self, max_elements: int = 50000) -> None:
		...
		# Chemical Initilization
        self.face_N = np.zeros(max_elements, dtype=np.float64)
        self.face_D = np.zeros(max_elements, dtype=np.float64)
        self.face_I = np.zeros(max_elements, dtype=np.float64)