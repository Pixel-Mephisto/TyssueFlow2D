...
from model.config import MeshConfig, PhysicsConfig, T1Config
...
from model.physics import run_physics_step

class MainDashboardApp:
    def __init__(self, root: tk.Tk) -> None:
        self.root = root
        self.root.title("TyssueFlow Pro - Framework Node")
    ...
    def _build_sidebar_widgets(self):
        m_box = self.sidebar.add_label_frame(" 1. Lattice Configuration ")
        ...
        p_box = self.sidebar.add_label_frame(" 2. Physics & Energy Weights ")
        ...
        s_box = self.sidebar.add_label_frame(" 3. Live Viscous Solver ")
        self.sidebar.add_slider(s_box, "Time Step (dt):", self.var_dt, 0.01, 0.20, 0.01)
        ...
        ttk.Checkbutton(toggle_frame, text="Live Forces Vectors", variable=self.show_f_total).pack(side=tk.RIGHT, padx=5)
        ...
        t1_box = self.sidebar.add_label_frame(" 4. Topological Surgery (T1) ")
        ...
        self.t1_btn = self._create_btn(t1_box, "✂️ Resolve T1s", self.manual_resolve_t1)
        self.t1_btn.pack(fill=tk.X, pady=(10, 5))

        self.energy_readout = tk.StringVar(value="System Energy: --")
        s = ttk.Style()
        s.configure('TReadout.TLabel', background=self.styles['btn_bg'], foreground=self.styles['accent_blue'], font=("Courier", 16, "bold"))
        ttk.Label(self.sidebar.inner_frame, textvariable=self.energy_readout, style='TReadout.TLabel', justify="center", padding=15).pack(fill='x', side=tk.BOTTOM, pady=(15, 0), padx=15)

    def _embed_graph_canvas(self):
        self.fig, (self.ax_mesh, self.ax_energy) = plt.subplots(2, 1, figsize=(10, 14), gridspec_kw={'height_ratios': [3, 1]})
        self.fig.patch.set_facecolor(self.styles['bg_main'])
        self.ax_mesh.set_facecolor(self.styles['bg_panel'])
        self.ax_energy.set_facecolor(self.styles['bg_panel'])
        self.ax_mesh.axis('off'); self.ax_energy.axis('off')
        
        self.fig.subplots_adjust(left=0.02, right=0.98, top=0.95, bottom=0.06, hspace=0.25)
    ...
    def _read_ui_parameters(self) -> bool:
        try:
            # FIX: Force UI to steal focus, committing any manual typed entries before processing
            self.root.focus_set()
    ...
    def trigger_initialization(self):
        try:
            ...
            e_init = run_physics_step(self.tissue, self.p_cfg)
            self.current_time = 0.0
            self.energy_history = [e_init]
            self.time_history = [self.current_time]
            
            self.energy_readout.set(f"System Energy: {e_init:,.2f}")
            self._sync_and_redraw("Mesh Loaded")
    ...
    def step_one_frame(self):
        ...
        swaps, energy = advance_simulation_frame(self.tissue, self.p_cfg, self.t1_cfg)
        
        self.current_time += self.p_cfg.dt
        self.energy_history.append(energy)
        self.time_history.append(self.current_time)
        
        self.energy_readout.set(f"System Energy: {energy:,.2f}")
        self._sync_and_redraw("Solver Relaxing")
    ...
    def _sync_and_redraw(self, status_msg):
        ...
        if len(self.energy_history) > 0:
            self.ax_energy.plot(self.time_history, self.energy_history, color=self.styles['accent_blue'], lw=3)
            
        # NATIVE LATEX RENDERING FOR HAMILTONIAN
        eq_text = r"$\mathcal{H} = \sum_{\alpha} \frac{K_A}{2}(A_{\alpha} - A_0)^2 + \sum_{\alpha} \frac{\Gamma_P}{2}(P_{\alpha} - P_0)^2 + \sum_{\langle i,j \rangle} \Lambda l_{ij}$"
        self.ax_energy.set_title("System Energy Curve\n" + eq_text, color=self.styles['fg_text'], fontdict={'weight': 'bold', 'size': 13}, pad=12)
        
        self.ax_energy.set_xlabel("Time (t)", color=self.styles['fg_text'], fontsize=12)
        self.ax_energy.tick_params(colors=self.styles['fg_text'], labelsize=10)
        
        for spine in self.ax_energy.spines.values(): spine.set_color('#444444')
    ...
    def manual_resolve_t1(self):
        if not self.tissue: return
        self._read_ui_parameters()
        from model.physics import resolve_t1_fast_numba
        
        dx = self.tissue.vert_x[self.tissue.edge_trgt[:self.tissue.num_edges]] - self.tissue.vert_x[self.tissue.edge_srce[:self.tissue.num_edges]]
        dy = self.tissue.vert_y[self.tissue.edge_trgt[:self.tissue.num_edges]] - self.tissue.vert_y[self.tissue.edge_srce[:self.tissue.num_edges]]
        lengths = np.sqrt(dx**2 + dy**2)
        valid_lengths = lengths[lengths > 1e-6]
        min_len = np.min(valid_lengths) if len(valid_lengths) > 0 else 0.0
        
        total_swaps = 0
        for _ in range(5):
            swaps = resolve_t1_fast_numba(
                self.tissue.num_edges, self.tissue.vert_x, self.tissue.vert_y,
                self.tissue.edge_srce, self.tissue.edge_trgt, self.tissue.edge_twin,
                self.tissue.edge_next, self.tissue.edge_prev, self.tissue.edge_face,
                self.t1_cfg.threshold, self.t1_cfg.rest_length
            )
            if swaps == 0: break
            total_swaps += swaps

        if total_swaps > 0:
            messagebox.showinfo("Surgery Complete", f"Resolved {total_swaps} T1 transitions!\n\nSmallest edge before surgery: {min_len:.4f}\nYour Threshold (l₀): {self.t1_cfg.threshold:.4f}")
            self._sync_and_redraw("Manual T1 Surgery")
        else:
            messagebox.showwarning("Scan Complete", f"No edges found under threshold.\n\nSmallest edge found: {min_len:.4f}\nYour Threshold (l₀): {self.t1_cfg.threshold:.4f}")
